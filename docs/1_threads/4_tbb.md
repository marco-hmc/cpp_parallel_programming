...
TBB 提供了一套线程安全的容器，如 `tbb::concurrent_vector`、`tbb::concurrent_hash_map`、`tbb::concurrent_queue` 等。这些容器内部实现了细粒度锁或无锁机制，允许多个线程并发读写而无需外部同步，适合在多线程环境中作为共享数据结构使用。与标准库容器外加手动锁的方案相比，TBB 的并发容器通常具有更好的伸缩性和更低的竞争开销。
